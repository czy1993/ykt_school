...
// 获取当前时间
export function today(new_Date) {
...
export function haveChildMenu(name) {
  let a;
  function reduce(children, arr) {
    children.reduce((acc, cur) => {
      acc.push(cur.label);
      return acc;
    }, arr);
  }
  function recursive(item, name) {
    if (item.label === name) {
      if (item.children && item.children.length > 0) {
        return item.children.reduce((acc, cur) => {
          acc.push(cur.label);
          if (cur.children && cur.children.length > 0) {
            reduce(cur.children, acc);
          }
  ...
  return function callback(menu) {
    for (let item of menu) {
      if (a) {
        return a;
      }
      if ((a = recursive(item, name))) {
        return a;
      }
