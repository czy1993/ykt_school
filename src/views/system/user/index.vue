    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
          ...
          <el-col :span="12">
            <el-form-item label="工号" prop="userName">
              <el-input
                ...
                :disabled="title=='修改用户'?true:false"
                placeholder="请输入工号"
              />
            </el-form-item>
          </el-col>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item v-if="schoolShow" label="选择学校" prop="schoolId">
              <el-select
                v-model="form.schoolId"
                @change="get_user(form.schoolId)"
                :disabled="title=='修改用户'?true:false"
                placeholder="请选择"
              >
                <el-option
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="用户性别">
              <el-select v-model="form.sex" :disabled="title=='修改用户'?true:false" placeholder="请选择">
                <el-option
...
export default {
  ...
  created() {
    if (window.localStorage.getItem("schoolId") == "1") {
  ...
  methods: {
    ...
    getList() {
      ...
      listUser(this.addDateRange(this.queryParams, this.dateRange)).then(
        (response) => {
          ...
          this.userList.forEach((elenemt) => {
            this.School.forEach((item) => {
              if (elenemt.schoolId == item.id) {
                elenemt.school = item.name;
              }
            });
          });
    ...
    handleAdd() {
      ...
      this.get_user(window.localStorage.getItem("schoolId"));
      // this.getTreeselect();
    },
    /** 查询角色 **/
    get_user(schoolId) {
    ...
    handleResetPwd(row) {
      ...
      this.$confirm(
        `确定要修改“${
          row.userName ? row.userName : "选中用户"
        }”的密码, 是否继续?`,
        "提示",
        {
          confirmButtonText: "确定",
        ...
        .then(() => {
          ...
          resetUserPwd(data).then((response) => {
            if (response.code === 200) {
              this.msgSuccess("修改成功，新密码是：" + "000000");
            }
          });
        })
        .catch(() => {});
